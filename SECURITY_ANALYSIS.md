...
Under **PoLP-generated policies**, this defense-in-depth model is further strengthened by
restricting thread-creation authority:

- `InputStreamCounter` provides the **intrinsic resource guard** — always active regardless
  of policy, it limits *how much data* each thread can read from any single JAR entry.
- **PoLP thread-creation restrictions** (e.g., denying `RuntimePermission("createPlatformThread")`
  to untrusted code) limit the number of concurrent `JarFile` opens, reducing the `N × 2 GiB`
  multi-thread attack surface documented in the residual-risk table below.
- Together: SecurityManager controls *who* may open a JAR entry and *how many* threads can do
  so concurrently; `InputStreamCounter` limits *how much data* each thread can read.

Administrators deploying PoLP-generated policies should ensure that thread-creation grants are
observation-derived and not over-broad; see the "Multiple concurrent opens" residual risk below.
...
| 6 | Confused-deputy in trusted paths (N-8, N-10; consolidated) | Reflection/MethodHandle and `<clinit>`/bootstrap can reach trusted code that uses unrestricted `doPrivileged` | Disallow unrestricted `doPrivileged` on security-sensitive trusted paths; keep stack-intersection guard; **under PoLP-generated policies the attack surface is substantially bounded: the observation-derived grant ceiling limits which operations trusted code can perform even when reachable via a confused-deputy path; see "N-8 Residual Risk Mitigation via Principle of Least Privilege (PoLP) Policy Generation"** |
| 7 | Finalizer/Cleaner context escape (N-9) | Both Finalizer and Cleaner threads now run under `neverPrivileged` (Issue #129), blocking `Subject.doAsPrivileged()` escalation. `neverPrivileged` alone cannot prevent unrestricted `AccessController.doPrivileged()` calls, which still intersect the callback class's `ProtectionDomain`. PoLP observation bounds those class-level grants, preventing unobserved operations. **OPERATIONALLY MITIGATED** — neverPrivileged (blocks Subject escalation) + PoLP (blocks unrestricted doPrivileged escalation) | Ensure both gates are active: deploy with `neverPrivileged` on Finalizer/Cleaner threads (Issue #129) and generate PoLP policies that reflect observed callback behaviour; avoid sensitive finalizer/cleaner work and prefer explicit `close()`; **see "Analysis: Finalizer and Cleaner Thread Execution Contexts (N-9)" for the full dual-layer mitigation model and PoLP risk-assessment table** |
| 8 | Runtime attach — residual risk substantially mitigated (N-11) | Attach is permission-gated, but over-grants, inactive SM, or OS compromise remain; **three independent gates provide defense-in-depth: (1) `AttachPermission("attachVirtualMachine")` blocks the JVM from accepting the attach request; (2) `-XX:+DisableAttachMechanism` removes the attach listener entirely at OS level; (3) `LoadClassPermission` under PoLP blocks class loading even if attach succeeds and the agent JAR is appended — agent codebases absent from the PoLP observation window receive no `LoadClassPermission` grants and therefore cannot load classes even if `AttachPermission` is bypassed** | Keep attach grants narrow; use `-XX:+DisableAttachMechanism` where feasible; **deploy PoLP-generated policies so that agent codebases are structurally excluded from `LoadClassPermission` grants; all three gates are independent — an attacker who bypasses one still faces the remaining two; see "Analysis: Runtime Agent Attachment and PoLP Policy Defense (N-11)"** |
...
| 11 | Module system residuals (N-15) | Runtime topology mutation is gated (`mutateModuleTopology`; see §10) and inspection is gated (`readModuleTopology`; see §11), but startup flags `--add-opens`, `--add-exports`, and `--add-modules` bypass these runtime permission checks at JVM bootstrap time and cannot be retroactively constrained by runtime permission gates | Treat startup flags as explicit trust-boundary decisions at deployment time; no runtime revocation possible; see §10 (`RuntimePermission("mutateModuleTopology")`) and §11 (`RuntimePermission("readModuleTopology")`) for the runtime API gating details |
