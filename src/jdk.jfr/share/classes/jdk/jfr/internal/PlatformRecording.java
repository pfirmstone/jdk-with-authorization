...
public final class PlatformRecording implements AutoCloseable {
    ...
    private void transferChunks(WriteableUserPath userPath) throws IOException {
        ...
        try (ChunksChannel cc = new ChunksChannel(chunks); FileChannel fc = FileChannel.open(userPath.getReal(), StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING,  StandardOpenOption.CREATE)) {
            // Mitigate races against other processes
            FileLock l = fc.tryLock();
            if (l == null) {
                Logger.log(LogTag.JFR, LogLevel.INFO, "Dump operation skipped for recording \"" + name + "\" (" + id + "). File " + userPath.getRealPathText() + " is locked by other dump operation or activity.");
                return;
            }
